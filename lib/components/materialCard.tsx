    ...
    return (
        <Link href={`${pathName}/${m.id}`}>
            <div className="bg-white rounded-xl shadow-md hover:shadow-xl transition-shadow duration-300 overflow-hidden hover:scale-105 transform transition-transform">
                ...
                <div className="p-4 flex flex-col h-[160] md:h-[210] lg:h-[236]">
                    <h3 className="text-md md:text-lg font-semibold text-gray-800 mb-2 text-pretty">{m.name}</h3>
                    <div className="flex flex-col flex-grow">
                        <p className="text-sm text-gray-600 mb-3 line-clamp-3 md:line-clamp-2">{m.short_description[0].toUpperCase() + m.short_description.slice(1)}</p>
                        {m.categories_array && (
